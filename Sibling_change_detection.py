...
for ix1, ix2 in zip(np.arange(115, 160), np.arange(116, 161)):
    if ix1 in np.arange(80, 111):
        pass
    else:
        ifg1 = ifgs[ix1]
        ifg2 = ifgs[ix2]
        ...
        np.save(f"timeseries/dropped_mask_{ix1}_{ix2}_{datetime.strftime(datetime.now(), '%Y%m%d_%H%M%S')}.npy", dropped_mask)

        df.to_csv(f"timeseries/IFG_{ix1}_{ix2}_{datetime.strftime(datetime.now(), '%Y%m%d_%H%M%S')}.csv")
        RF = joblib.load("RF_20230215_121116.jbl")

        features = df[["jk_std", "jk_bias", "amp_mean", "max_amp_diff", "poi_diff", "apparent_coherence"]]
        ...
        np.save(f"timeseries/probabilities_{ix1}_{ix2}_{datetime.strftime(datetime.now(), '%Y%m%d_%H%M%S')}.npy", probs_arr)
        np.save(f"timeseries/predictions_{ix1}_{ix2}_{datetime.strftime(datetime.now(), '%Y%m%d_%H%M%S')}.npy", predictions_arr)
