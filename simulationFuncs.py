...
def pixelCoherence(im1, im2, sibling_i, sibling_j):
    ...
    coherence_num = np.nansum(
        im1[sibling_i, sibling_j] * im2[sibling_i, sibling_j].conjugate()
    )
    coherence_den = np.sqrt(
        np.nansum(im1[sibling_i, sibling_j] * im1[sibling_i, sibling_j].conjugate())
        * np.nansum(im2[sibling_i, sibling_j] * im2[sibling_i, sibling_j].conjugate())
    )
...
def coherenceMatrix(i, j, SHP_i, SHP_j, arr):
    ...
    for i_ix, j_ix in zip(i_, j_):
        mat[i_ix, j_ix] = pixelCoherence(
            arr[i_ix, :, :],
            arr[j_ix, :, :],
            sibling_i.astype(int),
            sibling_j.astype(int),
        )
    return mat


def bootstrapCoherence(
    im1, im2, SHP_i, SHP_j, max_iter=1000, n_siblings=None, progress=True, track=False
):
    import random
    ...
    random_ixs = np.array(
        random.choices(np.arange(n_siblings), k=n_siblings * max_iter)
    ).reshape((max_iter, n_siblings))
...
def bootstrapTimeseries(
    i, j, sib_i, sib_j, ifgs, baseline=1, max_iter=1000, track=False, disable=False
):
    ...
    if baseline == 0:
        ...
        BS_timeseries = np.empty(
            (ceil(ifgs.shape[0] / baseline) - 1, max_iter), dtype=np.complex64
        )
    sibi = sib_i[:, i, j]
    sibj = sib_j[:, i, j]
    ...
    for ix, d_ix in enumerate(
        tqdm(range(ifgs.shape[0])[: -baseline : int(baseline)], disable=disable)
    ):
        im1 = ifgs[d_ix]
        im2 = ifgs[d_ix + int(baseline)]

        if track:
            coherences, random_ixs = bootstrapCoherence(
                im1,
                im2,
                sibi.flatten(),
                sibj.flatten(),
...
def coherenceTimeseries(i, j, sib_i, sib_j, ifgs, baseline=1, progress=False):
    ...
    for d_ix in tqdm(
        range(ifgs.shape[0])[: -baseline : int(baseline)], disable=~progress
    ):
        im1 = ifgs[d_ix]
        im2 = ifgs[d_ix + int(baseline)]
        coherence = pixelCoherence(im1, im2, sibi.flatten(), sibj.flatten())
...
def flatBSWrapper(
    ifgs, ij, SHP_i, SHP_j, start=140, end=None, n=5, central=False, seed=None
):
...
def flatWrapper(ifgs, ij, SHP_i, SHP_j):
    ...
    sib_i = SHP_i[~np.isnan(SHP_i[:, i, j]) * ~np.isnan(SHP_j[:, i, j]), i, j].astype(
        int
    )
    sib_j = SHP_j[~np.isnan(SHP_i[:, i, j]) * ~np.isnan(SHP_j[:, i, j]), i, j].astype(
        int
    )
...
def plotBootstrapTimeseries(
    BS,
    overlay=None,
    overlay_label=None,
    overlay_lim=[None, None],
    save=False,
    title=None,
    ax=None,
    cbar_max=500,
    ...
    if log:
        h = ax.hist2d(
            (np.resize(np.arange(BS.shape[0]).flatten(), BS.shape[::-1]).T).flatten(),
            BS.flatten(),
            bins=[np.arange(BS.shape[0]), bins],
            norm=mpl.colors.LogNorm(vmax=cbar_max),
            cmap=cmap,
        )
    else:
        h = ax.hist2d(
...
def plotBootstrapTimeseriesBox(
    BS,
    overlay=None,
    overlay_label=None,
    overlay_lim=[None, None],
...
def box(data, ax=None):
    ...
    for d in np.arange(data.shape[0]):
        ax.plot(
            [d, d], [up[d], lo[d]], color="black", ls="-", marker="_", markersize=10
        )
        ax.scatter(
            np.ones(np.sum(outliers[d])) * d,
            data[d][outliers[d]],
            marker="o",
            facecolor="none",
            color="black",
...
def interactiveCoherence(
    ...
    _, _, bar_container = ax3.hist(
        data, bins=HIST_BINS, lw=1, ec="white", fc="green", alpha=0.5
    )
...
def complexCoherenceAIC_TS(BS):
    ...
    for coherences in tqdm(BS):
        ...
        AICs.append(
            np.min([GMM.aic(X) - GMM1.aic(X) for GMM in GMMs])
        )  # - GMM1.aic(X))
...
def magnitudeCoherenceAIC_TS(BS):
    ...
    for coherences in BS:
        ...
        GMMs = [
            GaussianMixture(n_components=n).fit(coherences_) for n in np.arange(2, 6, 1)
        ]
        AICs.append(
            np.min([GMM.aic(coherences_) for GMM in GMMs]) - GMM1.aic(coherences_)
        )
...
def polarInteractiveClusters(X, ylim=1, clusters=None):
    ...
    lines = [
        ax.plot(theta[clusters == c], r[clusters == c], ".", markersize=3)[0]
...
def quickCoherence(
    ...
    subset_j_ix = np.repeat(
        np.arange(0, m - 1, 1)[:, np.newaxis], n - 1, axis=1
    ).T.flatten()

    for pq in tqdm(
        np.dstack((subset_i_ix, subset_j_ix))[0], desc=desc, disable=~progress
    ):
        p, q = pq
        i_start = subset_i[p]
        i_end = subset_i[p + 1]
        ...
        im1_subset, im2_subset = np.full(
            subset_sib_j.shape, np.nan, dtype=np.complex64
        ), np.full(subset_sib_j.shape, np.nan, dtype=np.complex64)

        mask = ~np.isnan(subset_sib_i) * ~np.isnan(subset_sib_j)

        im1_subset[mask] = im1[
            subset_sib_i[mask].astype(int), subset_sib_j[mask].astype(int)
        ]
        im2_subset[mask] = im2[
            subset_sib_i[mask].astype(int), subset_sib_j[mask].astype(int)
        ]
...
def runRF(
    ...
    importance = pd.DataFrame(
        np.dstack((np.array(list(train_features)), rf.feature_importances_))[0],
...
def assignWeights(data, mask=None, bins=50):
    ...
    weights[weights > 0] = (
        np.max(weights[weights > 0]) / weights[weights > 0]
    ) / np.sum(np.max(weights[weights > 0]) / weights[weights > 0])
...
def manipulateSiblings(
    coord1, coord2, d1, d2, data, sib_i, sib_j, n_change, smooth=True, seed=None
):
    """
    ...
    if n_change is None:
        n_change = np.random.randint(
            low=np.round(np.min(np.array([ph_1.shape[1], ph_2.shape[1]])) * 0.25),
            high=np.min(np.array([ph_1.shape[1], ph_2.shape[1]])),
            size=1,
        )[0]
    ...
    indices1 = np.array(
        random.sample(range(ph_1.shape[1]), n_change)
    )  # Indices to be changed in ph_1

    if np.isclose(coord1, coord2).all():
        indices2 = indices1.copy()
    else:
        indices2 = np.array(
            random.sample(range(ph_2.shape[1]), n_change)
        )  # Indices to be changed into of ph_2
    ...
    ph_change[np.ix_(np.arange(ph_change.shape[0]), indices1)] = ph_2[
        np.ix_(np.array([d2 - 1, d2]), indices2)
    ]
...
def generateSims(coords1, coords2, d1s, d2s, data, sib_i, sib_j, n_changes):
    ...
    for n, (coord1, coord2, d1, d2, n_change) in enumerate(
        ...
        if (coord1 == coord2).all():
            ph_change, actual_coherence, apparent_coherence = manipulateSiblings(
                coord1,
                coord2,
                d1,
                d2,
...
def meanDiff(arr, kernel_size=11):
    ...
    smoothed = signal.fftconvolve(
        arr, (np.ones(kernel_size) / kernel_size)[:, None, None], mode="same"
    )
...
def sampleUniform(arr, mask=None, bins=50, N=1000):
    ...
    selected_values = np.full(
        (np.max(bin_numbers), N_each_bin), dtype=float, fill_value=np.nan
    )

    for bin_number in tqdm(
        np.arange(1, np.max(bin_numbers) + 1),
        desc=f"Looping through bins ({N_each_bin = })",
    ):
    ...
    selected_values = selected_values[
        (~np.isnan(selected_values) * (selected_values > 0))
    ].flatten()
...
def jackknifeImage(ifg1, ifg2, sib_i, sib_j):
    ...
    def jack(s):
        ...
        out = quickCoherence(
            ifg1,
            ifg2,
            sib_i[mask].reshape((sib_i.shape[0] - 1, *sib_i.shape[1:])),
            sib_j[mask].reshape((sib_j.shape[0] - 1, *sib_j.shape[1:])),
            progress=False,
        )
...
def jackknifeMulticore(ifg1, ifg2, sib_i, sib_j, n_cores=6):
    ...
    results = pool.map(
        partial(coherence_exclude, ifg1, ifg2, sib_i, sib_j), np.arange(sib_i.shape[0])
    )
...
def generateMetricsIFG(ifg1, ifg2, sib_i, sib_j):
    ...
    image_with_siblings[0, mask] = ifg1[
        sib_i[mask].astype(int), sib_j[mask].astype(int)
    ]
    image_with_siblings[1, mask] = ifg2[
        sib_i[mask].astype(int), sib_j[mask].astype(int)
    ]
    ...
    print("max_amp_diff")
    max_amp_diff = np.nanmax(
        abs(image_with_siblings[0, 1:]) - abs(image_with_siblings[1, 1:]), axis=0
    ).flatten()
    print("poi_diff")
...
def postProcess_df(df_fn):
    ...
    plt.figure(2)
    plt.scatter(
        df["n_siblings"], abs(df["actual_coherence"] - df["apparent_coherence"]), s=1
    )
...
def filterPredictions(predictions, SHP_i, SHP_j, threshold):
    ...
    mask_siblings[mask] = predictions[
        (SHP_i[mask]).astype(int), (SHP_j[mask]).astype(int)
    ]

    if threshold % 1 == 0:
        return (
            mask_siblings,
            (np.nansum(mask_siblings, axis=0) > threshold) & predictions,
        )
    else:
        return (
            mask_siblings,
            (np.nansum(mask_siblings, axis=0) / np.sum(mask, axis=0) > threshold)
            & predictions,
        )


def animate_3d(data, vmin=0, vmax=1, cmap="gray"):
    ...
    def update(frame):
        ...
        # Plot the current slice of the data array
        plt.pcolormesh(data[frame], cmap=cmap, vmin=vmin, vmax=vmax)

        # Set the plot title to show the current slice index
        plt.title("Slice {}".format(frame))
...
def animate_slider(data, vmin=0, vmax=1, cmap="gray"):
    """
    Create an animation of each slice of a boolean 3D numpy array going down axis=0.

    Parameters:
        data (numpy.ndarray): The boolean 3D numpy array to animate.
    """
    ...
    def update(frame):
        ...
        # Get the current slice of the data array
        slice_data = data[frame]

        # Set the extent of the plot
        x_extent = [0, slice_data.shape[1]]
        y_extent = [0, slice_data.shape[0]]

        # Plot the current slice of the data array using pcolormesh
        plt.pcolormesh(slice_data, vmin=vmin, vmax=vmax, cmap=cmap)
    ...
    # Create the animation object
    anim = FuncAnimation(fig, update, frames=data.shape[0])

    # Create a slider widget for the slice index
    slice_slider_ax = fig.add_axes([0.2, 0.05, 0.6, 0.03])
    slice_slider = Slider(
        slice_slider_ax, "Slice", 0, data.shape[0] - 1, valinit=0, valstep=1
    )

    # Set the update function for the slice slider
    def update_slice(val):
        anim.event_source.stop()
        anim.event_source.start()
        anim.frame_seq = anim.new_frame_seq(slice(val))
        anim._draw_next_frame = lambda *args: None
        anim.frame_seq = anim.new_frame_seq(slice(val, val + 1))
        anim._blit_clear(self._drawn_artists, self._blit_cache)
        anim._drawn_artists = []
...
def selection_window_metric(
    ifgs, SHP_i=None, SHP_j=None, start=0, end=1, progress=False
):
    """
    ifgs: 3d array of interferograms with axis 0 as time.
    SHP_i, SHP_j: 3d arrays for the coords of the siblings in the i and j direction respectively.
    start: Start index of the interferograms to be used in the sibling selection.
    end: End index of the interferograms to eb sued in the sibling selection.
    """
    ...
    weighted_coh_diffs = np.empty((end - start, *ifgs.shape[1:]))
    ...
    for ix in tqdm(np.arange(start, end), disable=disable):

        coherence = quickCoherence(ifgs[ix], ifgs[ix + 1], SHP_i, SHP_j, progress=False)

        arr_sibs = np.empty_like(SHP_i, dtype=np.complex64)

        arr_sibs[~mask] = np.nan

        arr_sibs[mask] = coherence[SHP_i[mask].astype(int), SHP_j[mask].astype(int)]

        weighting = 1 / np.sqrt(sigma_CR(arr_sibs[1:]))

        weighting = weighting / np.nansum(weighting, axis=0)

        weighted_coh_diff = abs(coherence) - abs(
            np.nansum(arr_sibs[1:] * weighting, axis=0)
        )

        weighted_coh_diffs[ix - start] = weighted_coh_diff

    return weighted_coh_diffs
...
def importData():
    ...
    return wdir, ifgs, dates, SHP_i, SHP_j, coherence_2015


def multilook(im, fa, fr):
    """FROM RAPIDSAR
    Averages image over multiple pixels
    ...
    aa = np.zeros((int(na / fa), nr))
    for k in range(fa):
        aa = aa + im[k::fa, :]
    imout = np.zeros((int(na / fa), int(nr / fr)))
    for k in range(fr):
        imout = imout + aa[:, k::fr]
    return imout / fa / fr


def multilook_nan(im, fa, fr):
    """FROM RAPIDSAR
